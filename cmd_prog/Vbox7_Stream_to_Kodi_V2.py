...
create_pl = {'jsonrpc':'2.0','method':'Playlist.Insert','params':[1,0,{'file':'plugin://plugin.video.vbox7/?url=VOD&mode=7&name=%D0%97%D0%B0%D1%80%D0%B5%D0%B4%D0%B8+%D0%B2%D0%B8%D0%B4%D0%B5%D0%BE+%D0%BF%D0%BE+%D0%BD%D0%B5%D0%B3%D0%BE%D0%B2%D0%BE%D1%82%D0%BE+ID'}],'id':55}
x = requests.post(url, json = create_pl, auth = (kodi_username, kodi_password), headers=headers)

# Open item in playlist(you will need input VideoID in popup windows)
...
# Input VideoID
time.sleep(2)   # Wait 2sec to open windows and after this input VideoID
play_v = {'jsonrpc':'2.0','method':'Input.SendText','params':[url_id],'id':36}
x = requests.post(url, json = play_v, auth = (kodi_username, kodi_password), headers=headers)